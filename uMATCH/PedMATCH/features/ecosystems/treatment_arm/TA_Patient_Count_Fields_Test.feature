...
Feature: pMATCH Treatment Arm API Tests that focus on numPatientsAssigned and maxPatientAllowed fields
  ...
  Scenario: TA_PC1. New Treatment Arm with numPatientsAssigned field that has minus value should fail
    ...
    And set template treatment arm json field: "numPatientsAssigned" to value: "-30" in type: "int"
  ...
  Scenario Outline: TA_PC2. New Treatment Arm, float value in numPatientAssigned should be trimmed to int value
    ...
    And set template treatment arm json field: "numPatientsAssigned" to value: "<floatValue>" in type: "float"
    When posted to MATCH newTreatmentArm
    ...
    |APEC1621-PC2-1|23.8           |23.0           |
    |APEC1621-PC2-2|0.6837         |0.0            |

  Scenario Outline: TA_PC3. New Treatment Arm, numPatientAssigned should not have limit
    Given template treatment arm json with an id: "<treatmentArmID>"
    And set template treatment arm json field: "numPatientsAssigned" to value: "<value>" in type: "int"
    When posted to MATCH newTreatmentArm
    Then success message is returned:
    Then retrieve the posted treatment arm from API
...
#    Given template treatment arm json with a new unique id
#    And set template treatment arm json field: "numPatientsAssigned" to value: "75" in type: "int"
