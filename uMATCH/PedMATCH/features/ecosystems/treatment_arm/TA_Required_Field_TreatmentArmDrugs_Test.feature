...
Feature: Treatment Arm API Tests that focus on "treatmentArmDrugs" and "exclusionDrugs" field
  ...
  Scenario: New Treatment Arm with duplicated drug entities should be ignored
    Given template json with a new unique id
    ...
    And add drug with name: "AZD9291" pathway: "EGFR" and id: "781254" to template json
    When posted to MATCH newTreatmentArm
    Then the treatment arm with id: "saved_id" and version: "2016-06-03" return from API has "1" drug (name:"AZD9291" pathway: "EGFR" and id: "781254")
  ...
  Scenario: Update Treatment Arm with duplicated drug entities should be ignored
    Given template json with a new unique id
  ...
  Scenario: Update Treatment Arm with add same drug from "treatmentArmDrugs" to "exclusionDrugs" field should fail
    Given template json with a new unique id
    And set template json field: "version" to string value: "2010-06-03"
    And clear list field: "treatmentArmDrugs" from template json
    And clear list field: "exclusionDrugs" from template json
    And add drug with name: "AZD9291" pathway: "EGFR" and id: "781254" to template json
    When posted to MATCH newTreatmentArm
  ...
  Scenario: Update Treatment Arm with add same drug from "exclusionDrugs" to "treatmentArmDrugs" field should fail
    Given template json with a new unique id
    And set template json field: "version" to string value: "2010-06-03"
    And clear list field: "treatmentArmDrugs" from template json
    And clear list field: "exclusionDrugs" from template json
