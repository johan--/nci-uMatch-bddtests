...
Feature: Treatment Arm API Tests that focus on "stratumId" field
  ...
  Scenario Outline: TA_SI1. Treatment arm with empty or null stratumID should fail
    Given template json with an id: "APEC1621-SI1-1", stratum_id: "<stratumID>" and version: "2016-06-03"
  ...
  Scenario: TA_SI2. Treatment arm without stratumID field should fail
    Given template json with an id: "APEC1621-SI2-1", stratum_id: "straID" and version: "2016-06-03"
    Then remove field: "stratumId" from template json
    When posted to MATCH newTreatmentArm
