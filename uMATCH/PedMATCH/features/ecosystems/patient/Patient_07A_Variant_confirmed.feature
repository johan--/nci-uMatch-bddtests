...
#  variant_confirmed:
      ...
      #variant confirm service doesn't work yet
...
#  variant_file_confirmed:
  Scenario Outline: PT_VC06. variant report confirm message with invalid patient_id should fail
    Given template variant report confirm message for patient: "<value>", it has molecular_id: "MOI1", analysis_id: "ANI1" and status: "confirm"
...
#      |null           |can't be blank        |
  ...
  Scenario Outline: PT_VC08. variant report confirm message with invalid molecular_id should fail
    Given template variant report confirm message for patient: "PT_VC08_VRUploaded", it has molecular_id: "<MOI>", analysis_id: "PT_VC08_VRUploaded_ANI1" and status: "confirm"
    When put to MATCH variant report confirm service, returns a message that includes "<message>" with status "Failure"
    Examples:
      |MOI            |message                    |
#      |               |can't be blank             |
#      |null           |can't be blank             |
  ...
  Scenario Outline: PT_VC09. variant report confirm message with invalid analysis_id should fail
    Given template variant report confirm message for patient: "PT_VC09_VRUploaded", it has molecular_id: "PT_VC09_VRUploaded_MOI1", analysis_id: "<ANI>" and status: "confirm"
    When put to MATCH variant report confirm service, returns a message that includes "<message>" with status "Failure"
    Examples:
      |ANI            |message                    |
#      |               |can't be blank             |
...
##  Test patient: PT_VC10_VRUploadedANIExpired: 1. PT_VC10_VRUploadedANIExpired(_SEI1, _MOI1, _ANI1), 2. PT_VC10_VRUploadedANIExpired(_SEI1, MOI1, _ANI2)
    ...
    Given template variant report confirm message for patient: "<patient_id>", it has molecular_id: "<moi>", analysis_id: "<ani>" and status: "confirm"
    When put to MATCH variant report confirm service, returns a message that includes "<message>" with status "Failure"
    Examples:
...
#    Then this variant has confirmed field: "false" and comment field: "Tests"
  ...
  #             PT_VC15_PathAssayDoneVRUploadedToReject VR uploaded PT_VC15_PathAssayDoneVRUploadedToReject(_SEI1, _MOI1, _ANI1), Assay result received (_SEI1, _BC1), Pathology is confirmed (_SEI1)
    ...
    |PT_VC15_VRUploadedPathConfirmed          |PT_VC15_VRUploadedPathConfirmed_MOI1           |PT_VC15_VRUploadedPathConfirmed_ANI1           |confirm    |TISSUE_VARIANT_REPORT_CONFIRMED    |
    |PT_VC15_VRUploadedAssayReceived          |PT_VC15_VRUploadedAssayReceived_MOI1           |PT_VC15_VRUploadedAssayReceived_ANI1           |confirm    |TISSUE_VARIANT_REPORT_CONFIRMED    |
    |PT_VC15_PathAssayDoneVRUploadedToConfirm |PT_VC15_PathAssayDoneVRUploadedToConfirm_MOI1  |PT_VC15_PathAssayDoneVRUploadedToConfirm_ANI1  |confirm    |PENDING_CONFIRMATION               |
    |PT_VC15_PathAssayDoneVRUploadedToReject  |PT_VC15_PathAssayDoneVRUploadedToReject_MOI1   |PT_VC15_PathAssayDoneVRUploadedToReject_ANI1   |reject     |TISSUE_VARIANT_REPORT_REJECTED     |
