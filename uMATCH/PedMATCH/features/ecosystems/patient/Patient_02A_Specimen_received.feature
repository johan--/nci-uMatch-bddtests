...
Feature: Receive NCH specimen messages and consume the message within MATCH:
  ...
  Scenario: PT_SR03. "Blood" specimen received message with surgical_event_id should fail
    ...
    Then set patient message field: "surgical_event_id" to "SEI-001"
  ...
  Scenario: PT_SR04. "Tissue" specimen received message without surgical_event_id should fail
    ...
    When posted to MATCH patient trigger service, returns a message that includes "Surgical event is missing" with status "Failure"
  ...
  Scenario: PT_SR07. Return error when specimen received message is received for non-existing patient
    ...
    When posted to MATCH patient trigger service, returns a message that includes "Patient (patient_id: PT-SpecimenTest-NonExistingPatient) does not exist" with status "Failure"
  ...
  Scenario Outline: PT_SR08. Return error message when invalid type (other than BLOOD or TISSUE) is received
    ...
    Then set patient message field: "type" to "<specimen_type_value>"
  ...
  Scenario Outline: PT_SR09. Only ONE tissue can be received using same surgical event id
    Given template specimen received message in type: "<specimen_type>" for patient: "PT_SR09_Registered"
    ...
    Examples:
    |specimen_type  |collectTime              |receivedTime             |waitTime |status |message                                                                |
    |TISSUE         |2016-04-28T15:17:11+00:00|2016-04-29T15:17:11+00:00|0        |Success|Message has been processed successfully                                |
    |TISSUE         |2016-04-30T15:17:11+00:00|2016-05-01T15:17:11+00:00|5        |Failure|Tissue specimen using this surgical event id has already been received |
  ...
  Scenario Outline: PT_SR11. Return error message when study_id is invalid
    ...
    Then set patient message field: "<field>" to "<value>"
    When posted to MATCH patient trigger service, returns a message that includes "<message>" with status "Failure"
    Examples:
