...
#    |PT_ETE13_OnTA2 |N        |
  ...
  @patients_p1
  Scenario Outline: PT_ETE16. patient can be set to request assignment (rebiopsy=N) properly
    Given patient: "<patient_id>" with status: "<current_status>" on step: "<current_step_number>"
  ...
  @patients_p2
  Scenario Outline: PT_ETE17. patient can only be set to request assignment(rebiopsy=N) when patient is on certain status
    Given patient: "<patient_id>" with status: "<current_status>" on step: "<current_step_number>"
    Then requests assignment for this patient with re-biopsy: "N", step number: "<next_step_number>"
