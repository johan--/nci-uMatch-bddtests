...
Feature: Patient GET service tests (valid results)
  ...
  Scenario Outline: PT_GV12. Service with id should return valid projections
    ...
    When GET from MATCH patient API, http code "200" should return
    Then the response type should be "Hash"
    And response should have 3 fields
    And response should have field "<field1>"
    And response should have field "<field2>"
    And response should have field "<field3>"
    Examples:
      | service         | field1                       | value1                               | field2         | field3              |
      | events          | entity_id                    | PT_GVF_TsShipped                     | event_message  | event_data          |
      | variant_reports | variant_report_received_date | PT_GVF_RARebioY_ANI1                 | tsv_file_name  | total_amois         |
      | variants        | uuid                         | 5f4be20f-dc2b-4f62-9767-25ad7a320b0c | amois          | variant_type        |
      | assignments     | assignment_date              | PT_GVF_VrAssayReady_ANI1             | step_number    | sent_to_cog_date    |
      | shipments       | shipped_date                 | PT_GVF_OnTreatmentArm_MOI1           | carrier        | shipment_type       |
      |                 | registration_date            | PT_GVF_RequestNoAssignment           | current_status | current_step_number |
...
#    Examples:
  ...
  Scenario Outline: PT_GV23. Service with patient_id should return valid result with parameters
    ...
    Examples:
      ...
      | PT_GVF_TsReceivedTwice | specimens | specimen | surgical_event_id | PT_GVF_TsReceivedTwice_SEI2 |
  ...
  Scenario Outline: PT_GV31. Service with patient_id and id should return valid result
    ...
    Examples:
      ...
      | PT_GVF_VrAssayReady  | analysis_report       | PT_GVF_VrAssayReady_ANI1 | variant_report    |
      | PT_GVF_VrAssayReady  | analysis_report_amois | PT_GVF_VrAssayReady_ANI1 | total_amois       |
...
#
  Scenario Outline: PT_GV32. Service with patient_id and id should return valid projections
    Given patient GET service: "<service>", patient id: "PT_GVF_VrAssayReady", id: "<id>"
    Then add projection: "<field1>" to patient GET url
    Then add projection: "<field2>" to patient GET url
    Then add projection: "<field3>" to patient GET url
    When GET from MATCH patient API, http code "200" should return
    ...
    And response should have field "<field3>"
    Examples:
      | service               | id                       | field1    | field2        | field3       |
      | specimens             | PT_GVF_VrAssayReady_SEI1 | entity_id | event_message | event_data   |
      | qc_variant_reports    | PT_GVF_VrAssayReady_ANI1 |           |               |              |
      | variant_file_download | PT_GVF_VrAssayReady_ANI1 |           |               |              |
