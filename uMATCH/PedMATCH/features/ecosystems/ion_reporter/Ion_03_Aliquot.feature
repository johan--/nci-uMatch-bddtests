...
@ion_reporter_p1
  ...
  Scenario: ION_AQ02. for sample control specimen, aliquot service will generate tsv and bai files and upload to S3

  Scenario: ION_AQ03. for patient specimen, aliquot service will generate tsv and bai files and upload to S3

  Scenario: ION_AQ04. aliquot service will store generated sample control variants to database (with correct analysis_id), once process done

  Scenario: ION_AQ05. aliquot service will send variant files uploaded message (with correct analysis_id) to patient ecosystem once process done

  Scenario: ION_AQ06. aliquot service will return error if site-ion_reporter_id pair in message body is invalid

  Scenario: ION_AQ07. aliquot service will return error if any file value(vcf, bam, qc) is missing

  Scenario: ION_AQ08. extra key-value pair in the message body should NOT fail
