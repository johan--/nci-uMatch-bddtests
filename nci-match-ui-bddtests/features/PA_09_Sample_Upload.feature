...
Feature: MATCHKB-542. Users can upload patient sample files.
  ...
  Scenario Outline: As Sender type user, link to upload is <visibility> to a patient of status <status>
    ...
    Then The "Upload new sample files" link is "<visibility>"
    Examples:
  ...
  Scenario: As a privileged user I can cancel upload
    ...
    And I can see that some files have not been uploaded for the Surgical Event
    Then The "Upload new sample files" link is "visible"
    And The "Upload new sample files" button is "enabled"
    And I click on the "Upload new sample files" button
    And I can see the "Upload BAM files and Variant ZIP files" dialog
    ...
    Then The "Upload new sample files" button is "enabled"
  ...
  @ui_p2 @flyboy
  Scenario: As a privileged Mocha Sender I can upload a blood variant report
    Given I clear the file from S3 under reporter "IR_MDA00", mol_id "ION_AQ03_BdShipped_BD_MOI1", analysis_id "ION_AQ03_An123"
    And I am logged in as a "VR_Sender_mda" user
    When I go to patient "ION_AQ03_BdShipped" details page
    And I click on the Blood Specimens tab
    Then The "Upload new sample files" link is "visible"
    And The "Upload new sample files" link is "enabled"
    And I can click on the "Upload new sample files" link
    And I can see the "Upload BAM files and Variant ZIP files" dialog
    And The "Start Upload" button looks "disabled"
    Then I select an Ion Reporter "mda - IR_MDA00"
    And I enter Analysis ID "ION_AQ03_An123"
    And I make all elements visible
    And I press "Select Variant ZIP File" file button to upload "vcf_sample.zip" file
    And I press "Select DNA BAM File" file button to upload "dna_sample.bam" file
    Then The "Start Upload" button looks "enabled"
